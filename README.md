...
The default `susfile` generated by `opensus init` includes common recon/web tools such as nmap, gobuster, nikto, and sqlmap, plus Metasploit helper tools for module search and check workflows.
...
- `metasploit_check(module, rhosts, rport)` → `msfconsole -q -x "use <module>; set RHOSTS <rhosts>; set RPORT <rport>; check; exit -y"`

These are designed for evidence-first module discovery and vulnerability validation before exploitation.
