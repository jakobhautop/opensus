...
pub fn tools_for_agent(agent: &str, cfg: &Susfile) -> Vec<Value> {
    match agent {
        "main_agent" => vec![
            ...
            tool_write_plan(),
            tool_spawn_agent(),
            tool_no_args("read_analyst_count", "Read current active analyst count"),
        ],
        "strategist_agent" => vec![
            tool_no_args("read_plan", "Read plan.md"),
            tool_write_plan(),
            tool_no_args(
                "read_tool_data",
...
fn tool_spawn_agent() -> Value {
    json!({"type":"function","function":{"name":"spawn_agent","description":"Spawn an agent by role name: analyst, strategist, or reporter. For analyst include task_id.","parameters":{"type":"object","properties":{"name":{"type":"string","enum":["analyst","strategist","reporter"]},"task_id":{"type":"string"}},"required":["name"]}}})
}
